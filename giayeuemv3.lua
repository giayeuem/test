-- GIAYEUEM V3: trial lifecycle upgrade; V2 group/API/gear preserved.
...
-- V3: shared trial state. Missing UI means unknown, never permission to attack.
local TrialV3 = {
    Enabled = true, Generation = 0, ControlEpoch = 0, CancelledRounds = {},
    Config = { TrialTimeout = 420, PvPTimeout = 180, TargetTimeout = 45,
        RetryDelay = 3, TrialRadius = 1500, TargetRadius = 1500 },
}
function TrialV3.Timer()
    local gui = Players.LocalPlayer:FindFirstChild("PlayerGui")
    local main = gui and gui:FindFirstChild("Main")
    if not main then return nil end
    local hud = main:FindFirstChild("TopHUDList")
    local function visible(timer)
        if not timer then return nil end
        local ok, value = pcall(function() return timer.Visible end)
        if ok then return value == true end
        return nil
    end
    local raid = visible(hud and hud:FindFirstChild("RaidTimer"))
    local legacy = visible(main:FindFirstChild("Timer"))
    -- A hidden duplicate must not mask an active timer in the other HUD.
    if raid == true or legacy == true then return true end
    if raid == false or legacy == false then return false end
    return nil
end
function TrialV3.Border()
    ...
    if field and field:IsA("BasePart") then return field end
    return nil
end
function TrialV3.InArena(root)
    local field = TrialV3.Border()
    if not field or not root then return false end
    local p = field.CFrame:PointToObjectSpace(root.Position)
    local half = field.Size * 0.5
    return math.abs(p.X) <= half.X and math.abs(p.Y) <= half.Y
        and math.abs(p.Z) <= half.Z
end
local raceTrialNames = {
    Human = "Trial of Strength", Mink = "Trial of Speed", Fishman = "Trial of Water",
    Skypiea = "Trial of the King", Ghoul = "Trial of Carnage", Cyborg = "Trial of the Machine",
}
-- Resolve on every access: no startup wait for six maps, no stale location cache.
local races_trial_place = setmetatable({}, { __index = function(_, race)
    local origin = workspace:FindFirstChild("_WorldOrigin")
    local locations = origin and origin:FindFirstChild("Locations")
    local name = raceTrialNames[race]
    return locations and name and locations:FindFirstChild(name) or nil
end })
function TrialV3.Snapshot()
    ...
    local race = raceValue and raceValue.Value
    local location = races_trial_place[race]
    local timer = TrialV3.Timer()
    local field = TrialV3.Border()
    local phase = "unknown"
    if not hum or hum.Health <= 0 or not root then phase = "dead"
    elseif timer == false then phase = "idle"
    elseif timer == true then
        if field and field.Transparency == 0 then
            if TrialV3.InArena(root) then phase = "pvp" end
        elseif field and location and location.Parent
            and (root.Position - location.Position).Magnitude < TrialV3.Config.TrialRadius then
            phase = "trial"
        end
    end
    if TrialV3.Phase ~= phase or TrialV3.Character ~= char or TrialV3.Race ~= race then
        local previousPhase = TrialV3.Phase
        TrialV3.Generation = TrialV3.Generation + 1
        -- Unknown/dead observations don't replenish the current phase budget.
        if phase == "idle" or TrialV3.Character ~= char or TrialV3.Race ~= race then
            TrialV3.Deadlines = {}
        end
        TrialV3.Phase, TrialV3.Character, TrialV3.Race = phase, char, race
        if (previousPhase == "trial" or previousPhase == "pvp") and TrialV3.OnTransition then
            TrialV3.OnTransition()
        end
    end
    TrialV3.Deadlines = TrialV3.Deadlines or {}
    if (phase == "trial" or phase == "pvp") and not TrialV3.Deadlines[phase] then
        TrialV3.Deadlines[phase] = tick() + (phase == "trial"
            and TrialV3.Config.TrialTimeout or TrialV3.Config.PvPTimeout)
    end
    return { phase = phase, character = char, root = root, race = race, location = location,
        generation = TrialV3.Generation, deadline = TrialV3.Deadlines[phase] }
end
function TrialV3.Allowed(phase, generation)
    local s = TrialV3.Snapshot()
    return TrialV3.Enabled and s.phase == phase
        and (not generation or generation == s.generation)
        and s.deadline ~= nil and tick() < s.deadline, s
end
...
function getplayers(all)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if plr ~= Players.LocalPlayer and hum and hum.Health > 0 and TrialV3.InArena(root)
            and (all or (plr.Name ~= mainAccountName and noideaforname(plr))) then
            result[char] = true
        end
    end
    return result
...
function localDoorState()
    ...
    -- Unknown timer blocks V3 activation until the HUD is available.
    local timerVisible = TrialV3.Timer() ~= false
...
function isInsideOwnTrial()
    local s = TrialV3.Snapshot()
    return s.phase == "trial" or s.phase == "pvp"
end

function forceMatchedAccountToTemple()
    if not TrialV3.Enabled or TrialV3.Timer() ~= false then return false end
...
function waitForSharedFireTime(fireAt, epoch)
    while true do
        if epoch and (not TrialV3.Enabled or epoch ~= TrialV3.ControlEpoch) then return false end
...
function scheduleWorkspaceRound(command)
    if not TrialV3.Enabled then return false end
    local epoch = TrialV3.ControlEpoch
    ...
    if TrialV3.CancelledRounds[roundId] then return false end
    ...
    task.spawn(function()
        local roundOk = pcall(function()
        if not waitForSharedFireTime(fireAt, epoch) then
            if scheduledRoundId == roundId then scheduledRoundId = "" end
            return
        end
        ...
        if TrialV3.Enabled and epoch == TrialV3.ControlEpoch and validGroup
            and doorState.nearDoor and not doorState.timerVisible
            and v3ServerNow() <= (tonumber(command.expires_at) or fireAt + 10) then
            status("Activating Race V3 from shared workspace time")
            for index = 1, V3_FIRE_COUNT do
                if not TrialV3.Enabled or epoch ~= TrialV3.ControlEpoch then break end
            ...
            if fired then
                -- Local dispatch only; server acceptance is not proven here.
        ...
        if scheduledRoundId == roundId then scheduledRoundId = "" end
        if not roundOk then status("Trial V3: countdown error - lock released") end
    end)
    return true
end
    ...
    local ok = pcall(function()
        ...
        if isUper and isMyUpgearTurn() then
            ...
            if not command then
                ...
                if ownReady then status("At race door - waiting Main countdown") end
            end
        end
    end)
    activatingAbility = false
    if not ok or not TrialV3.Enabled or epoch ~= TrialV3.ControlEpoch then
        if command and command.round_id then
            TrialV3.CancelledRounds[tostring(command.round_id)] = true
        end
        return false
    end
...
-- V3 trial combat owner. Group scheduling, training and gear purchase stay in V2.
TrialV3.TargetBackoff = setmetatable({}, { __mode = "k" })
function TrialV3.Cleanup()
    SourceCombatState.currentMob = nil
    AttackConfig.AutoClickEnabled = false
    AttackConfig.AttackPlayers = false
    pcall(function() module:cancelTopos() end)
end
function TrialV3.Stop()
    TrialV3.Enabled = false
    _G.SHOULDSPAMSKILLS = false
    TrialV3.ControlEpoch = TrialV3.ControlEpoch + 1
    if scheduledRoundId ~= "" then TrialV3.CancelledRounds[scheduledRoundId] = true end
    readySent = false
    TrialV3.Generation = TrialV3.Generation + 1
    if TrialV3.StopWater then pcall(TrialV3.StopWater) end
    TrialV3.Cleanup()
end
function TrialV3.Start()
    -- Don't replace a live owner; cancellation must unwind before a new session.
    if TrialV3.Busy then return false end
    TrialV3.ControlEpoch = TrialV3.ControlEpoch + 1
    TrialV3.Generation = TrialV3.Generation + 1
    TrialV3.Deadlines = {}
    TrialV3.TargetBackoff = setmetatable({}, { __mode = "k" })
    TrialV3.Enabled = true
    return true
end
TrialV3.OnTransition = function()
    if TrialV3.StopWater then pcall(TrialV3.StopWater) end
    TrialV3.Cleanup()
end
function TrialV3.TargetAlive(target, s, pvp)
    if not target or not target.Parent or not s.root then return false end
    local hum = target:FindFirstChildOfClass("Humanoid")
    local root = target:FindFirstChild("HumanoidRootPart")
    if not hum or hum.Health <= 0 or not root then return false end
    if pvp then return target ~= s.character and TrialV3.InArena(root) end
    return s.location and s.location.Parent
        and (root.Position - s.location.Position).Magnitude < TrialV3.Config.TargetRadius
end
function TrialV3.SelectTarget(s, pvp)
    local candidates = {}
    if pvp then
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= Players.LocalPlayer and plr.Character then
        ...
        local enemies = workspace:FindFirstChild("Enemies")
        candidates = enemies and enemies:GetChildren() or {}
    end
    local best, nearest = nil, math.huge
    for _, target in ipairs(candidates) do
        if tick() >= (TrialV3.TargetBackoff[target] or 0) and TrialV3.TargetAlive(target, s, pvp) then
            local distance = (target.HumanoidRootPart.Position - s.root.Position).Magnitude
            if distance < nearest then best, nearest = target, distance end
        end
    end
    return best
end
function TrialV3.RunCombat(pvp)
    if TrialV3.Busy then return end
    local phase = pvp and "pvp" or "trial"
    local allowed, initial = TrialV3.Allowed(phase)
    if not allowed then return end
    if pvp and not (isMain and isMyUpgearTurn()) then return end
    if not pvp and initial.race ~= "Human" and initial.race ~= "Ghoul" then return end
    TrialV3.Busy = true
    local ok, err = pcall(function()
        local target, targetDeadline, lastEquip = nil, 0, 0
        while true do
            local active, s = TrialV3.Allowed(phase, initial.generation)
            if not active or (pvp and not (isMain and isMyUpgearTurn())) then break end
            if target and (tick() >= targetDeadline or not TrialV3.TargetAlive(target, s, pvp)) then
                TrialV3.TargetBackoff[target] = tick() + TrialV3.Config.RetryDelay
                target = nil
                TrialV3.Cleanup()
            end
            if not target then
                target = TrialV3.SelectTarget(s, pvp)
                targetDeadline = tick() + TrialV3.Config.TargetTimeout
            end
            if target then
                if tick() - lastEquip >= 0.5 then
                    if pvp then module:eqMelee() else module:eq() end
                    module:haki()
                    lastEquip = tick()
                end
                -- Equip/haki can yield; revalidate before claiming attack/movement.
                local stillActive, current = TrialV3.Allowed(phase, initial.generation)
                if not stillActive or not TrialV3.TargetAlive(target, current, pvp)
                    or (pvp and not (isMain and isMyUpgearTurn())) then break end
                SourceCombatState.currentMob = target
                AttackConfig.AttackPlayers = pvp
                AttackConfig.AutoClickEnabled = true
                module:holdTopos(target.HumanoidRootPart.CFrame * CFrame.new(0, pvp and 3 or 30, 0))
            end
            task.wait(0.05)
        end
    end)
    -- Always clear our ownership, including errors, respawn and phase transition.
    TrialV3.Cleanup()
    TrialV3.Busy = false
    if not ok then
        status("Trial V3: combat error; retry after backoff")
        task.wait(TrialV3.Config.RetryDelay)
    end
end
function TrialV3.MoveRace(race)
    local allowed, s = TrialV3.Allowed("trial")
    if not allowed then return end
    local map = workspace:FindFirstChild("Map")
    if not map then return end
    local node, offset
    if race == "Mink" then
        local trial = map:FindFirstChild("MinkTrial")
        node, offset = trial and trial:FindFirstChild("Ceiling"), CFrame.new(0, -20, 0)
    elseif race == "Skypiea" then
        local trial = map:FindFirstChild("SkyTrial")
        local model = trial and trial:FindFirstChild("Model")
        node, offset = model and model:FindFirstChild("FinishPart"), CFrame.new()
    elseif race == "Cyborg" then
        local trial = map:FindFirstChild("CyborgTrial")
        node, offset = trial and trial:FindFirstChild("Floor"), CFrame.new(0, 500, 0)
    end
    if node and node:IsA("BasePart") then
        local active = TrialV3.Allowed("trial", s.generation)
        if active then module:holdTopos(node.CFrame * offset) end
    else
        pcall(function() module:cancelTopos() end)
    end
end
getgenv().__KaitunTrialV3 = TrialV3
...
-- Movement-only races also release flight immediately when the phase changes.
task.spawn(function()
    while true do
        task.wait(0.1)
        pcall(function()
            local s = TrialV3.Snapshot()
            if s.deadline and tick() >= s.deadline and (s.phase == "trial" or s.phase == "pvp") then
                if TrialV3.StopWater then pcall(TrialV3.StopWater) end
                TrialV3.Cleanup()
            end
        end)
...
-- Trial Water: V2 skill scheduler retained; holds default to 0.5 seconds.
-- Banana decompilation is a reference, not proof of original runtime behavior.
-- Separate function scope avoids accumulating Water's locals in the main chunk.
;(function()
local TrialWater = {}

TrialWater.Config = {
    Keys = { Z = true, X = true, C = true, V = true, F = true },
    ...
    Weapons = {
        { kind = "Melee", alt = "Fighting Style",
          keys = { Z = true, X = true, C = true },
          hold = { Z = 0.5, X = 0.5, C = 0.5, V = 0.5, F = 0.5 } },
        { kind = "Sword", keys = { Z = true, X = true, C = true, V = true, F = true },
          hold = { Z = 0.5, X = 0.5, C = 0.5, V = 0.5, F = 0.5 } },
        { kind = "Gun", keys = { Z = true, X = true, C = true, V = true, F = true },
          hold = { Z = 0.5, X = 0.5, C = 0.5, V = 0.5, F = 0.5 } },
        { kind = "Blox Fruit", keys = { Z = true, X = true, C = true, V = true, F = true },
          hold = { Z = 0.5, X = 0.5, C = 0.5, V = 0.5, F = 0.5 } },
    },
    ...
    RegisterGap     = 0.4,    -- [1] giãn cách giữa hai lần thử
    UseNativeFlight = true,   -- [6] false = quay lại kiểu set hrp.CFrame thô
    Debug           = false,
}

local TW = TrialWater.Config
...
local twLegacyBusy = false
local twSession = 0
local twTrialBusy = false
local twCanCast = function() return false end
local twPressedKeys = {}
...
local function twLog(...)
    if TW.Debug then print("[TrialWater]", ...) end
end

local function twStatus(text)
    if type(status) == "function" then pcall(status, text) end
end

local function twChar() return Players.LocalPlayer.Character end

local function twHRP()
    local c = twChar()
    return c and c:FindFirstChild("HumanoidRootPart")
end

local function twHum()
    local c = twChar()
    return c and c:FindFirstChildOfClass("Humanoid")
end


-- Mọi đường nhấn phím đều đi qua finally-release, kể cả lỗi / dừng giữa hold.
local function twReleaseKeys()
    for key in pairs(twPressedKeys) do
        local ok = pcall(function()
            VirtualInputManager:SendKeyEvent(false, key, false, game)
        end)
        if ok then twPressedKeys[key] = nil end
    end
    return next(twPressedKeys) == nil
end

local function twWaitWhile(seconds, canContinue)
    local deadline = tick() + seconds
    while tick() < deadline do
        if not canContinue() then return false end
        task.wait(math.min(0.03, math.max(0, deadline - tick())))
    end
    return canContinue()
end

local function twSendHeldKey(key, hold, canContinue)
    if not twReleaseKeys() then return false end
    ...
    local released = twReleaseKeys()
    if not ok then twLog("skill error:", err) end
    if not released then twStatus("Trial Water: chưa nhả được phím; đang thử lại") end
    return ok and completed and released
end

-- NameWeapon — quét Backpack trước rồi Character, khớp theo ToolTip.
-- [4] "Melee" và "Fighting Style" được coi là một, giống module:eqMelee.
local function twNameWeapon(entry)
    local function scan(container)
        if not container then return nil end
        for _, v in ipairs(container:GetChildren()) do
            if v:IsA("Tool") then
                local tip = tostring(v.ToolTip or "")
                if tip == entry.kind or (entry.alt and tip == entry.alt) then
                    return v
                end
            end
        end
    ...
    return scan(Players.LocalPlayer:FindFirstChild("Backpack")) or scan(twChar())
end

-- equiptool — bản gốc bỏ qua khi Humanoid.Sit nên ngồi thuyền là bó tay.
local function twEquip(name)
    if not name then return false end
    local char, hum = twChar(), twHum()
...
-- Khung Skills có thể chỉ xuất hiện sau lần equip đầu tiên.
-- Giới hạn bootstrap mỗi phiên; không suy ra tần suất Banana gốc từ mã giải mã.
local function twTryRegister(toolName)
    local tries = twRegTries[toolName] or 0
    if tries >= TW.RegisterTries then return false end
    local last = twRegAt[toolName] or 0
    if tick() - last < TW.RegisterGap then return false end
    twRegTries[toolName] = tries + 1
    twRegAt[toolName] = tick()
    twEquip(toolName)
    twLog("dựng khung Skills cho", toolName, "lần", tries + 1)
    return true
end

-- Tìm một chiêu sẵn sàng của vũ khí. Sẵn sàng = title trắng (đã đủ mastery)
-- và thanh cooldown rỗng. [3] Không còn nhánh "thanh đầy -> thoát cả lượt".
local function twPickReadySkill(tool, entry)
    local gui = Players.LocalPlayer:FindFirstChild("PlayerGui")
    local main = gui and gui:FindFirstChild("Main")
    ...
    if not folder then
        return nil, twTryRegister(tool.Name)
    end
    twRegTries[tool.Name] = nil

    local selected = entry.keys or TW.Keys
    for _, key in ipairs(TW.KeyOrder) do
        local frame = folder:FindFirstChild(key)
        if frame and frame:IsA("Frame") and selected[key] then
            local title = frame:FindFirstChild("Title")
            local cd    = frame:FindFirstChild("Cooldown")
            if title and title:IsA("TextLabel") and cd then
                local isTransform = TW.SkipTransformation
                    and string.find(tostring(title.Text), "Transformation") ~= nil
                local pendingUntil = twPending[frame]
                if pendingUntil and (cd.Size ~= TW_READY_SIZE or tick() >= pendingUntil) then
                    twPending[frame] = nil
                end
                if not isTransform and not twPending[frame]
                    and title.TextColor3 == TW_WHITE
                    and cd.Size == TW_READY_SIZE then
                    return frame
                end
            end
        end
...
local function twFireOneSkill()
    ...
    for _, entry in ipairs(TW.Weapons) do
        ...
        local tool = twNameWeapon(entry)
        if tool then
            local frame, registered = twPickReadySkill(tool, entry)
            if registered then return false end -- chỉ bootstrap một tool mỗi lượt
            if frame and twCanCast() and twEquip(tool.Name) then
                local castCharacter = twChar()
                local key = frame.Name
                local hold = tonumber(entry.hold and entry.hold[key])
                    or tonumber(TW.Hold[key]) or 0.5
                hold = TW.FastMode and 0.05 or math.clamp(hold, 0, 5)
                local function canContinue()
                    return twCanCast() and twChar() == castCharacter
                        and tool.Parent == castCharacter
                ...
                if not canContinue() then return false end
                twPending[frame] = tick() + hold + TW.CooldownAckTimeout
                local sent = twSendHeldKey(key, hold, canContinue)
                if sent then twLog("bắn", tool.Name, "/", key, "hold", hold) end
                return sent
            end
        end
    end
...
local function twInstallHook()
    ...
    if type(getrawmetatable) ~= "function" or type(getnamecallmethod) ~= "function" then
        twStatus("Trial Water: thiếu hook API; không bắt đầu spam không định hướng")
        return false
    end

    local mt, wasReadonly
    local ok, err = pcall(function()
        mt = getrawmetatable(game)
        wasReadonly = type(isreadonly) ~= "function" or isreadonly(mt)
        ...
        twOldNamecall = previous
        twHookWrapper = { fn = wrapper, disable = function() enabled = false end }
        twHookOn = true
    end)
    if mt and wasReadonly ~= nil and type(setreadonly) == "function" then
        pcall(setreadonly, mt, wasReadonly)
    end
    if not ok then
        twStatus("Trial Water: cài hook thất bại")
        twLog("hook error:", err)
    end
    return ok and twHookOn
end

local function twRemoveHook()
    local own = twHookWrapper
    if own then own.disable() end -- wrapper còn trong chain cũng chỉ chuyển tiếp
    pcall(function()
        local mt = getrawmetatable(game)
        if own and mt.__namecall == own.fn then
            local readonly = type(isreadonly) ~= "function" or isreadonly(mt)
            if type(setreadonly) == "function" then setreadonly(mt, false) end
            local ok, err = pcall(function() mt.__namecall = twOldNamecall end)
            if type(setreadonly) == "function" then pcall(setreadonly, mt, readonly) end
            if not ok then twLog("unhook error:", err) end
        end
    end)
    twHookOn = false
    twHookWrapper = nil
...
-- [6] Ưu tiên flight controller sẵn có của file. Chỉ set CFrame thô khi bị
-- tắt tường minh, vì nó giành HumanoidRootPart với module:topos.
local function twMoveTo(cf)
    if TW.UseNativeFlight then
        pcall(function() module:holdTopos(cf) end)
        return
    end
    local hrp = twHRP()
    if not hrp then return end
    if not hrp:FindFirstChild("NoFall") then
...
local function twClearNoFall()
    local hrp = twHRP()
    local bv = hrp and hrp:FindFirstChild("NoFall")
    if bv then bv:Destroy() end
end

-- TeleportSeabeast2: quái ở tầng nước thường thì treo ngay trên nó, tầng bất
-- thường thì ép về một độ cao tuyệt đối. |y+60| <= 175  <=>  -235 <= y <= 115.
...
local function twFindBeast(loc)
    local folder = Workspace:FindFirstChild("SeaBeasts")
    if not folder or not loc then return nil end
    for _, v in ipairs(folder:GetChildren()) do
        if string.find(v.Name, "SeaBeast") then
            local hrp = v:FindFirstChild("HumanoidRootPart")
            local hp  = v:FindFirstChild("Health")
            if hrp and hp and hp.Value > 0
                and (hrp.Position - loc.Position).Magnitude <= TW.SearchRadius then
                return v
            end
        end
    end
...
-- V3 uses the shared timer/character/phase budget; unknown HUD pauses casting.
local function twTrialActive()
    return TrialV3.Allowed("trial")
end
...
local function twCleanup(prevSpam)
    ...
    pcall(twClearNoFall)
    pcall(function() module:cancelTopos() end)
    _G.SHOULDSPAMSKILLS = prevSpam == true and TrialV3.Enabled
    twRegTries = {}
    twRegAt    = {}
    twPending = setmetatable({}, { __mode = "k" })
end

--- Chạy trọn một lượt Trial of Water. Block cho tới khi quái chết, trial kết
--- thúc, rời quá xa, hoặc hết watchdog.
function TrialWater.RunTrial(trialLocation)
    if twTrialBusy or not trialLocation or not TrialV3.Allowed("trial") then return false end
    local beast = twFindBeast(trialLocation)
    if not beast then return false end

    local prevSpam = _G.SHOULDSPAMSKILLS == true
    twTrialBusy = true
    twRunning = true -- giành quyền trước khi yield, worker cũ sẽ thoát lượt
    twSession = twSession + 1
    local session = twSession
    local trialGeneration = TrialV3.Generation
    _G.SHOULDSPAMSKILLS = false
    local reason = "unknown"
    local ok, err = pcall(function()
        local handoffDeadline = tick() + 2
        while twLegacyBusy and twRunning and tick() < handoffDeadline do task.wait() end
        if twLegacyBusy then reason = "spam_handoff_timeout"; return end
        if not twRunning then reason = "stopped"; return end
        ...
        twCanCast = function()
            ...
            if not twRunning or twSession ~= session or tick() >= deadline
                or not TrialV3.Allowed("trial", trialGeneration) then return false end
            local hum, root = twHum(), twHRP()
            if twChar() ~= character or not hum or hum.Health <= 0 or not root then return false end
            if not trialLocation.Parent or not twTrialActive() then return false end
            local hp = beast.Parent and beast:FindFirstChild("Health")
            local target = beast.Parent and beast:FindFirstChild("HumanoidRootPart")
        ...
        while twRunning and twSession == session do
            ...
            twAimPos = nil
            local hum, root = twHum(), twHRP()
            if tick() >= deadline then reason = "watchdog_timeout"; break end
            if twChar() ~= character or not hum or hum.Health <= 0 or not root then
                reason = "character_lost"; break
            end
            if not trialLocation.Parent or not twTrialActive()
                or not TrialV3.Allowed("trial", trialGeneration) then reason = "trial_ended"; break end
            if (root.Position - trialLocation.Position).Magnitude > TW.LeashRadius then
                reason = "too_far"; break
            end
            if not beast.Parent then reason = "beast_gone"; break end
            local hp = beast:FindFirstChild("Health")
            local target = beast:FindFirstChild("HumanoidRootPart")
            if not hp or not target then reason = "target_lost"; break end
            if hp.Value <= 0 then reason = "beast_dead"; break end
    ...
    if not ok then reason = "error"; twLog("trial error:", err) end
    twCleanup(prevSpam)
    twTrialBusy = false
    twStatus("Trial Water: kết thúc (" .. reason .. ")")
    return reason == "beast_dead"
end
...
function TrialWater.Status()
    local wo = Workspace:FindFirstChild("_WorldOrigin")
    local locs = wo and wo:FindFirstChild("Locations")
    local loc = locs and locs:FindFirstChild("Trial of Water")
    ...
    return {
        ...
        beastHP     = beast and beast:FindFirstChild("Health") and beast.Health.Value or nil,
    }
end

TrialV3.StopWater = TrialWater.Stop
getgenv().__KaitunTrialWater = TrialWater

function TrialV3.Step()
    -- Keep V2's restoration path available even after the trial timer starts.
    local map = workspace:FindFirstChild("Map")
    if map and not map:FindFirstChild("Temple of Time") then
        local stash = ReplicatedStorage:FindFirstChild("MapStash")
        local temple = stash and stash:FindFirstChild("Temple of Time")
        if temple then temple.Parent = map end
    end
    local s = TrialV3.Snapshot()
    if not TrialV3.Enabled then return end
    if s.phase == "pvp" then
        if isMain and isMyUpgearTurn() then
            TrialV3.RunCombat(true)
        elseif isAlly and TrialV3.Allowed("pvp", s.generation) then
            local hum = s.character and s.character:FindFirstChildOfClass("Humanoid")
            if hum then hum.Health = 0 end
        end
    elseif TrialV3.Allowed("trial", s.generation) then
        if s.race == "Human" or s.race == "Ghoul" then
            TrialV3.RunCombat(false)
        elseif s.race == "Fishman" and not TrialV3.Busy then
            TrialV3.Busy = true
            local ok = pcall(TrialWater.RunTrial, s.location)
            if not ok then pcall(TrialWater.Stop) end
            TrialV3.Cleanup()
            TrialV3.Busy = false
        else
            local ok = pcall(TrialV3.MoveRace, s.race)
            if not ok then TrialV3.Cleanup() end
        end
    elseif s.deadline and tick() >= s.deadline then
        status("Trial V3: timeout - pause/resume to retry")
    else
        status("Trial V3: waiting for timer/map/character")
    end
end
...
spawn(function()
    while task.wait(0.1) do
        ...
        -- An active trial must not depend on pre-entry remote/full-moon checks.
        -- Unknown HUD blocks new Temple movement instead of assuming trial ended.
        if TrialV3.Timer() ~= false then
            local stepOk = pcall(TrialV3.Step)
            if not stepOk then
                pcall(TrialWater.Stop)
                TrialV3.Cleanup()
                TrialV3.Busy = false
            end
            task.wait(0.2)
        ...
        if not checktempledoor then
            ...
            if not workspace.Map:FindFirstChild("Temple of Time") then
                local templeRef = ReplicatedStorage.MapStash:FindFirstChild("Temple of Time")
                if templeRef then templeRef.Parent = workspace.Map end
            elseif TrialV3.Snapshot().phase == "pvp" then
                if isMain and isMyUpgearTurn() then
                    status("Trial V3: PvP")
                    TrialV3.RunCombat(true)
                elseif isAlly then
                    local s = TrialV3.Snapshot()
                    local hum = s.character and s.character:FindFirstChildOfClass("Humanoid")
                    if TrialV3.Allowed("pvp", s.generation) and hum then hum.Health = 0 end
                end
            else
                ...
                if race_trial_place and TrialV3.Allowed("trial") then
                    status("Doing trial")
                    local myrace = Players.LocalPlayer.Data.Race.Value
                    if myrace == "Human" or myrace == "Ghoul" then
                        TrialV3.RunCombat(false)
                    elseif myrace == "Fishman" then
                        if not TrialV3.Busy then
                            TrialV3.Busy = true
                            local twOk = pcall(TrialWater.RunTrial, race_trial_place)
                            -- RunTrial owns key/hook cleanup, even when its inner loop fails.
                            if not twOk then TrialWater.Stop() end
                            TrialV3.Cleanup()
                            TrialV3.Busy = false
                            task.wait(0.2)
                        end
                    else
                        local moved = pcall(TrialV3.MoveRace, myrace)
                        if not moved then TrialV3.Cleanup() end
                    end
                else
                    if TrialV3.Timer() == false then
                        local khang = nil
                        ...
                        repeat
                            ...
                            if not TrialV3.Enabled or TrialV3.Timer() ~= false then break end
                            if timeout > 300 then break end
                        until khang ~= nil
                        if not TrialV3.Enabled or TrialV3.Timer() ~= false then
                            readySent = false
                            pcall(function() module:cancelTopos() end)
                        elseif khang and getdis(khang.CFrame) < 1500 then
...
-- Worker cũ giữ nhịp ngoài TrialWater, nhưng phải nhường quyền ngay giữa lượt.
spawn(function()
    while task.wait(0.1) do
        if not twTrialBusy then twReleaseKeys() end
        if _G.SHOULDSPAMSKILLS and not twTrialBusy and TrialV3.Enabled then
            twLegacyBusy = true
            local character = Players.LocalPlayer.Character
            local epoch = TrialV3.ControlEpoch
            local function active()
                local hum = character and character:FindFirstChildOfClass("Humanoid")
                return _G.SHOULDSPAMSKILLS and not twTrialBusy
                    and TrialV3.Enabled and epoch == TrialV3.ControlEpoch
                    and Players.LocalPlayer.Character == character and hum and hum.Health > 0
            end
            local ok, err = pcall(function()
                ...
                for _, v in ipairs(weapon) do
                    ...
                    if not skills:FindFirstChild(v.Name) then EquipTool(v.Name) end
                end
                for _, v in ipairs(weapon) do
                    if not active() then return end
                    if v.Parent ~= character then EquipTool(v.Name) end
                    local main = Players.LocalPlayer.PlayerGui:FindFirstChild("Main")
                    local skills = main and main:FindFirstChild("Skills")
                    local ui = skills and skills:FindFirstChild(v.Name)
                    if ui then
                        for _, frame in ipairs(ui:GetChildren()) do
                            if not active() then return end
                            if isvalidnameui[frame.Name] then
                                local cd = frame:FindFirstChild("Cooldown")
                                local title = frame:FindFirstChild("Title")
                                if cd and title and title.TextColor3 == Color3.new(1, 1, 1)
                                    and cd.Size == UDim2.new(0, 0, 1, -1)
                                    and (frame.Name ~= "V" or not fruits[ui.Name]) then
                                    if not twSendHeldKey(frame.Name, 0.1, active) then return end
                                    if not twWaitWhile(1.5, active) then return end
                                end
                            end
                        end
                    end
            ...
            twReleaseKeys()
            twLegacyBusy = false
            if not ok then twLog("legacy spam error:", err) end
        end
    end
end)
...
function createUI()
    ...
    local Title = makeText(Header, "KAITUN V4 · V3", 18, C.Cyan, Enum.Font.GothamBold)
    ...
    local TrialButton = new("TextButton", {
        Name = "TrialV3Toggle", Text = "PAUSE TRIAL", TextSize = 13,
        Font = Enum.Font.GothamBold, TextColor3 = C.White,
        BackgroundColor3 = C.Header, BorderSizePixel = 0,
        Position = UDim2.new(0, 14, 0, 289), Size = UDim2.new(1, -28, 0, 28),
        Parent = Frame,
    })
    addCorner(TrialButton, 5)
    TrialButton.Activated:Connect(function()
        if TrialV3.Enabled then
            TrialV3.Stop()
            TrialButton.Text = "RESUME TRIAL"
        elseif TrialV3.Start() then
            TrialButton.Text = "PAUSE TRIAL"
        else
            TrialButton.Text = "WAIT FOR CLEANUP - RESUME"
        end
    end)
